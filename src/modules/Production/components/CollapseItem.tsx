...
const CollapseItem: FC<ICollapseItem> = ({ article, open }) => {
  ...
  return (
    <Box sx={styles.root}>
      <Box sx={styles.header} onClick={handleCollapse}>
        ...
        <IconButton size='small' sx={coll ? styles.up : styles.down} onClick={handleCollapse}>
      ...
      <Collapse in={coll}>
        <Box sx={styles.collWrapper}>
          ...
          {sortedSteps?.map((step, i) => (
            <StepCardItem key={step._id} reverse={i % 2 === 0} step={step} />
          ))}
        </Box>
      </Collapse>
      {!coll && <Divider />}
