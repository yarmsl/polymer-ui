...
const VacanciesDrawer = (): JSX.Element => {
  ...
  const leftColVacancies = useMemo(
    () => (Array.isArray(data) && data.length > 0 ? data.filter((_, i) => i % 2 === 0) : []),
    [data],
  );

  const rightColVacancies = useMemo(
    () => (Array.isArray(data) && data.length > 0 ? data.filter((_, i) => i % 2 === 1) : []),
    [data],
  );
  ...
  return (
    <>
      {isLoading ? (
        <Skeleton height='30px' variant='text' width='100%' />
      ) : (
        data &&
        data?.length > 0 && (
...
export default memo(VacanciesDrawer);
