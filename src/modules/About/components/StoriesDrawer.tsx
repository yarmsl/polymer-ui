...
const StoriesDrawer = (): JSX.Element => {
  ...
  const sortedStories = useMemo(
    () => (Array.isArray(data) && data.length > 0 ? [...data].sort((a, b) => a.from - b.from) : []),
    [data],
  );
  ...
  const rightColStories = useMemo(
    () => (sortedStories.length > 1 ? sortedStories?.filter((_, i) => i % 2 === 1) || [] : []),
    [sortedStories],
  );
  ...
  return (
    <>
      {isLoading ? (
        [0, 1, 2, 3].map((ph) => <Skeleton key={ph} height='30px' variant='text' width='100%' />)
      ) : (
